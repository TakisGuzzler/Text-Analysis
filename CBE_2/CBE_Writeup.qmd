---
title: "CBE 2"
author: "Amalia, Lauren, and Jenny"
date: last-modified
format:
  pdf:
    number-sections: true
    indent: true
    toc: true
    include-in-header:
      text: |
        \addtokomafont{disposition}{\rmfamily}
        \usepackage{fvextra}
        \DefineVerbatimEnvironment{Highlighting}{Verbatim}{breaklines,commandchars=\\\{\}}
execute:
  echo: false
---

\newpage

# Introduction

# Data

```{r, include=FALSE}
library(quanteda)
library(quanteda.textstats)
library(tidyverse)
library(sentimentr)
library(knitr)
library(gt)
library(syuzhet)
library(tidytext)
library(dplyr)
source("../R/helper_functions.R")
source("../R/keyness_functions.R")

```

```{r}
txt_1839 <- readLines("Text_Files/the_fall_of_the_house_of_usher_1839.txt")
txt_1842 <- readLines("Text_Files/the_masque_of_the_red_death_1842.txt")
txt_1941 <- readLines("Text_Files/a_painful_case_1914.txt")
txt_1930 <- readLines("Text_Files/a_rose_for_emily_1930.txt")
txt_1952 <- readLines("Text_Files/a_sound_of_thunder_1952.txt")
txt_1968 <- readLines("Text_Files/a_very_old_man_with_enormous_wings_1968.txt")
txt_1914 <- readLines("Text_Files/draculas_guest_1914.txt")
txt_1927 <- readLines("Text_Files/hills_like_white_elephants_1927.txt")
txt_1905 <- readLines("Text_Files/the_gift_of_the_magi_1905.txt")
txt_1940 <- readLines("Text_Files/the_man_who_killed_the_world_1940.txt")
txt_1915 <- readLines("Text_Files/the_metamorphasis_1915.txt")
txt_1884 <- readLines("Text_Files/the_necklace_1884.txt")
txt_1973 <- readLines("Text_Files/the_ones_who_walk_away_from_omelas_1973.txt")
txt_1926 <- readLines("Text_Files/the_rocking-horse_winner_1926.txt")
txt_1986 <- readLines("Text_Files/to_build_a_fire_1986.txt")

corpus_1986 <- txt_1986 %>%
corpus() %>%
tokens(what = "word", remove_punct = TRUE)

corpus_1926 <- txt_1926 %>%
corpus() %>%
tokens(what = "word", remove_punct = TRUE)

corpus_1973 <- txt_1973 %>%
corpus() %>%
tokens(what = "word", remove_punct = TRUE)

corpus_1884 <- txt_1884 %>%
corpus() %>%
tokens(what = "word", remove_punct = TRUE)

corpus_1915 <- txt_1915 %>%
corpus() %>%
tokens(what = "word", remove_punct = TRUE)

corpus_1940 <- txt_1940 %>%
corpus() %>%
tokens(what = "word", remove_punct = TRUE)

corpus_1905 <- txt_1905 %>%
corpus() %>%
tokens(what = "word", remove_punct = TRUE)

corpus_1927 <- txt_1927 %>%
corpus() %>%
tokens(what = "word", remove_punct = TRUE)

corpus_1914 <- txt_1914 %>%
corpus() %>%
tokens(what = "word", remove_punct = TRUE)

corpus_1968 <- txt_1968 %>%
corpus() %>%
tokens(what = "word", remove_punct = TRUE)

corpus_1952 <- txt_1952 %>%
corpus() %>%
tokens(what = "word", remove_punct = TRUE)

corpus_1839 <- txt_1839 %>%
corpus() %>%
tokens(what = "word", remove_punct = TRUE)

corpus_1941 <- txt_1941 %>%
corpus() %>%
tokens(what = "word", remove_punct = TRUE)

corpus_1930 <- txt_1930 %>%
corpus() %>%
tokens(what = "word", remove_punct = TRUE)

corpus_1842 <- txt_1842 %>%
corpus() %>%
tokens(what = "word", remove_punct = TRUE)

# short_stories_corpus <-   data.frame(doc_id = c("txt_1939", "txt_1942"), text = c(txt_1939, txt_1942)) %>%
#   mutate(text = preprocess_text(text)) %>%
#   corpus()
```

```{r}
#Tokens
token_1839 <- tokens(corpus_1839)
token_1842 <- tokens(corpus_1842)
token_1884 <- tokens(corpus_1884)
token_1905 <- tokens(corpus_1905)
token_1914 <- tokens(corpus_1914)
token_1915 <- tokens(corpus_1915)
token_1914 <- tokens(corpus_1914)
token_1952 <- tokens(corpus_1952)
token_1968 <- tokens(corpus_1968)
token_1973 <- tokens(corpus_1973)
token_1986 <- tokens(corpus_1986)
```

```{r}
#general token frequency
freq_1939 <- corpus_1939 %>%
dfm() %>%
textstat_frequency() %>%
as_tibble() %>%
dplyr::select(feature, frequency) %>%
rename(Token = feature, AF = frequency) %>%
mutate(RF = AF/sum(AF)*100)

freq_1942 <- corpus_1942 %>%
dfm() %>%
textstat_frequency() %>%
as_tibble() %>%
dplyr::select(feature, frequency) %>%
rename(Token = feature, AF = frequency) %>%
mutate(RF = AF/sum(AF)*100)

```
# Methods

# Results

# Discussion

# Acknowledgments {.appendix}

# Works Cited
