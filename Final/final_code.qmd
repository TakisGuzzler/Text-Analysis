...
## Lexical Complexity
We then sought to analyze three different common measurements for lexical complexity - lexical density, lexical diversity, and lexical sophistication (@ZHOU2023101262). We used all three measures because they offered an exhaustive exploration of lexical complexity. Specifically, average lexical density, average lexical diversity, and average lexical sophistication were used in order to draw comparison across time periods.
...
```{r fig.cap="Average Word Length of Short Stories from mid 1800s to mid 1900s", out.height = "30%"}
...
kable(pairwise_results, caption = "Pairwise T-Test Results for Lexical Density Across Time Periods")
...
```{r fig.cap="Increase in Relative Frequency of Grammatical Noun + Noun Pattern Over Time", out.height = "31%"}
...
```{r fig.cap="Decrease in Relative Frequency of Grammatical Adj + Noun Pattern Over Time", out.height = "31%"}
#run the following once (doesn't take long)
...
```{r fig.cap="Decrease in Relative Frequency of Prepositional Phrases with Nouns Over Time", out.height = "31%"}
...
We also found that the average lexical diversity decreased significantly when comparing corpora from the mid 1850s and mid 1950s for the CTTR and Yule's K metrics. However, we found that Uber's Index did not have a significant change between the time periods. It is interesting that the measurements for significance for CTTR and Uber's Index points us towards two different conclusions of the significance of lexical diversity in the differences between the texts over time. There are many explanations for this conclusion, however, including the different computations involved in the measurements and the small sample size for comparisons. 
Additionally, we found that lexical sophistication slightly decreased over time. We chose not to perform a t-test or significance test for this metric because our data was collected using TAALES (Tool for the Automatic Analysis of Lexical Sophistication) and thus was not preformed on the individual texts in a way that allowed for the use of a t-test. Future analyses could include computing significance tests to assess the usefulness of changes in lexical sophistication in observing changes in short stories over time.
