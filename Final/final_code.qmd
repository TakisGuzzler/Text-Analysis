...
nps_nouns_1850s <- read_csv("annotations_nps/simple_nouns/nps_1850s.csv")
nps_nouns_1875s <- read_csv("annotations_nps/simple_nouns/nps_1875s.csv")
nps_nouns_1900s <- read_csv("annotations_nps/simple_nouns/nps_1900s.csv")
nps_nouns_1925s <- read_csv("annotations_nps/simple_nouns/nps_1925s.csv")
nps_nouns_1950s <- read_csv("annotations_nps/simple_nouns/nps_1950s.csv")

nps_verbs_1850s <- read_csv("annotations_nps/simple_verbs/nps_simple_verbs_1850s.csv")
nps_verbs_1875s <- read_csv("annotations_nps/simple_verbs/nps_simple_verbs_1875s.csv")
nps_verbs_1900s <- read_csv("annotations_nps/simple_verbs/nps_simple_verbs_1900s.csv")
nps_verbs_1925s <- read_csv("annotations_nps/simple_verbs/nps_simple_verbs_1925s.csv")
nps_verbs_1950s <- read_csv("annotations_nps/simple_verbs/nps_simple_verbs_1950s.csv")

gen_table <- function(nps){
  nps_count <- nrow(nps)
  nps_AN <- nrow(nps %>% filter(pattern == 'AN'))
  nps_NN <- nrow(nps %>% filter(pattern == 'NN'))
  nps_P <- nrow(nps %>% filter(grepl('P', pattern)))
  nps_not <- nrow(nps %>% filter(!grepl('P', pattern), pattern != 'NN', pattern != 'AN'))
  pattern_counts <- data.frame(Pattern = c('AN', 'NN', 'P', '*'), AF= c(nps_AN, nps_NN, nps_P, nps_not)) %>% mutate(RF = AF/nrow(nps)*100)
  
  return(pattern_counts)
}

table_1850 <- gen_table(nps_nouns_1850s)
table_1875 <- gen_table(nps_nouns_1875s)
table_1900 <- gen_table(nps_nouns_1900s)
table_1925 <- gen_table(nps_nouns_1925s)
table_1950 <- gen_table(nps_nouns_1950s)

gen_df <- function(table_1850, table_1875, table_1900, table_1925, table_1950, pattern) {
  table_1850 <- table_1850 %>% filter(Pattern==pattern)
  table_1875 <- table_1875 %>% filter(Pattern==pattern)
  table_1900 <- table_1900 %>% filter(Pattern==pattern)
  table_1925 <- table_1925 %>% filter(Pattern==pattern)
  table_1950 <- table_1950 %>% filter(Pattern==pattern)
  
  pattern_counts <- data.frame(decade = c(1850, 1875, 1900, 1925, 1950),
                                RF = c(table_1850$RF, table_1875$RF, table_1900$RF, table_1925$RF, table_1950$RF))
  
  return(pattern_counts)
  
}

df_AN <- gen_df(table_1850, table_1875, table_1900, table_1925, table_1950, "AN")
df_NN <- gen_df(table_1850, table_1875, table_1900, table_1925, table_1950, "NN")
df_P <- gen_df(table_1850, table_1875, table_1900, table_1925, table_1950, "P")

```

```{r fig.cap="Decrease in Relative Frequency of Grammatical Adj + Noun Pattern Over Time", out.height = "30%"}

ggplot(df_AN, aes(x = decade, y = RF, group = 1)) +
  geom_line(color = "blue") +
  geom_point(color = "blue") +
  labs(
    x = "Year",
...
From Figure 3, the relative frequency of adjective-noun phrases slightly decreased from the mid-1800s to the mid-1900s, declining from a relative frequency of 8 in the 1850s to around 7.2 in the 1950s. This suggests a slight reduction in grammatical complexity, potentially reflecting a shift in writing style toward simpler and more direct expressions.

```{r fig.cap="Increase in Relative Frequency of Grammatical Noun + Noun Pattern Over Time", out.height = "30%"}

ggplot(df_NN, aes(x = decade, y = RF, group = 1)) +
  geom_line(color = "blue") +
  geom_point(color = "blue") +
  labs(
    x = "Year",
...
From Figure 4, the relative frequency of noun-noun phrases increased slightly over the same period, rising from approximately 12.5 in the 1850s to around 15 in the 1950s. This trend could suggest a shift towards more concise and direct language. 

```{r fig.cap="Decrease in Relative Frequency of Prepositional Phrases with Nouns Over Time", out.height = "30%"}

ggplot(df_P, aes(x = decade, y = RF, group = 1)) +
  geom_line(color = "blue") +
  geom_point(color = "blue") +
  labs(
    x = "Year",
