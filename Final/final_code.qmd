...
```{r fig.cap="Increase in Relative Frequency of Grammatical Noun + Noun Pattern Over Time", out.height = "32%"}
...
```{r fig.cap="Decrease in Relative Frequency of Grammatical Adj + Noun Pattern Over Time", out.height = "32%"}
#run the following once (doesn't take long)
...
```{r fig.cap="Decrease in Relative Frequency of Prepositional Phrases with Nouns Over Time", out.height = "32%"}
...
From our exploration, we found a decreasing trend in average sentence length and a slight decreasing trend in average word length over time. This could indicate that short stories have become easier to read as sentences have gotten shorter and words have gotten shorter. 

We also determined that the lexical density showed a small increase over time where the lexical diversity showed a decrease over time. However, there was a minimal difference between the two period's lexical density, which might a less significant impact. We also found that lexical diversity decreased significantly when comparing corpora from the mid 1850s and mid 1950s for CTTR and Yule's K metrics. We found that Uber's Index did not have a significant change between the time periods, but this could be due to the differing ways in which all the metrics are computed. 

Additionally, we found that lexical sophistication slightly decreased over time. We chose not to perform a t-test or significance test for this metric because our data was collected using TAALES (Tool for the Automatic Analysis of Lexical Sophistication). Future analyses could include computing t-tests to assess the significance of changes in lexical sophistication.
